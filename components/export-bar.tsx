...
function useShareActions({
  ...
  const exportPng = useCallback(async () => {
    ...
    try {
      ...
      link.download = `reporack-${repoNames.map((n) => n.replace("/", "-")).join("_")}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const shareOnX = useCallback(() => {
    ...
    const text = encodeURIComponent("Compare GitHub stars with RepoRack");
    window.open(
...
export function ExportBar({ repoNames, theme }: ExportBarProps) {
  ...
  const embedCode = useMemo(() => {
    ...
    const img = `https://reporack.dev/api/embed?repo=${encodeURIComponent(repo)}&theme=${encodeURIComponent(themeId)}`;
    const link = `https://reporack.dev/?repos=${encodeURIComponent(repo)}&theme=${encodeURIComponent(themeId)}`;
    return `[![RepoRack](${img})](${link})`;
  }, [repoNames, theme.id]);
